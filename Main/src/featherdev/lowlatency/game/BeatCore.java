...
class BeatCore implements IUpdateable, IDrawable, Poolable {
    ...
    public BeatCore() {
        ...
        ring = LowLatency.instance().textures.createSprite("game/ring");
        core = LowLatency.instance().textures.createSprite("game/ring");
    ...
    private void onHit(long songTimeMs) {
        ...
        float mwidth = LowLatency.instance().skin.getFont("naipol").getBounds(message).width;
    ...
    public void init(Beat b, Vector2 pos, float lifetime) {
        // calculate color based on beat energy
        beat = b;
        ec.set(getEnergyColor(b.energy));
        ring.setColor(ec);
        core.setColor(ec);

        // set shrink rate so that ring scales to match core in one lifeteim
        shrinkRate = (ring.getScaleX() - core.getScaleX()) / lifetime;
        timeToLive = lifetime;

        // center core and ring on position & set both of their internal origins
        position.set(pos);
        ring.setCenter(pos.x, pos.y);
        core.setCenter(pos.x, pos.y);
