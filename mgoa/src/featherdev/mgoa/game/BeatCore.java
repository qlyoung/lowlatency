...
import featherdev.mgoa.objects.Stats;

class BeatCore implements IUpdateable, IDrawable, Poolable {
	public enum CoreState { ALIVE, DYING, DEAD }
	public enum Accuracy { STELLAR, PERFECT, EXCELLENT, GOOD, ALMOST };
	public static OrthographicCamera cam = new OrthographicCamera(10, 6);
	...
	public boolean checkIsTouched(){
		if (Gdx.input.isTouched()){
			...
			return this.getHitbox().contains(touchpos.x, touchpos.y);
		}
		else
			return false;

	}
	public void onHit(long songTimeMs) {
		if (beenHit || state == CoreState.DYING || state == CoreState.DEAD)
			return;
